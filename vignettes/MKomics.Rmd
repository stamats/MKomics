...
for moderated t-test and one-way ANOVA 

- Hamming and the Levenshtein (edit) distance of strings as well as optimal 
alignment scores for global (Needleman-Wunsch) and local (Smith-Waterman) alignments 
with constant gap penalties (Merkl and Waack (2009), ISBN:978-3-527-32594-8).
...
code of the limma package.

### Moderated t-Test
We generate some artificial data and compute t-tests.
```{r}
## One-sample test
...
## MAD Matrix
In case of outliers the MAD (median absolute deviation = median of the absolute deviations from the median) may be useful as dispersion measure.
```{r}
madMatrix(t(M))
```
